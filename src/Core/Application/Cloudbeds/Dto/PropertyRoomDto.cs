...
public class PropertyRoomDto
{
    ...
    [JsonIgnore] public string Availability => RoomsAvailable < 5
        ? $"Only {RoomsAvailable} left"
        : $"{RoomsAvailable} {(IsSharedRoom ? "bed" : "room")}{(RoomsAvailable > 1 ? "s" : "")} available";

    [JsonIgnore] public int MaximumOccupancy => this.GetMaximumOccupancy();

    [JsonIgnore] public string CurrencySymbol { get; set; } = default!;

    [JsonIgnore] public DefaultIdType PropertyId { get; set; }

    [JsonIgnore] public string PropertyName { get; set; } = default!;
    ...
    [JsonIgnore] public int CloudbedsPropertyId { get; set; }

    [JsonIgnore] public string? PropertyImageUrl { get; set; }
}