...
public partial class Destinations
{
    ...
    private EntityServerTableContext<DestinationDto, Guid, DestinationViewModel> Context { get; set; } = default!;

    private EditContext? EditContext { get; set; }

    private EntityTable<DestinationDto, Guid, DestinationViewModel> _table = default!;
    ...
    {
        Context = new EntityServerTableContext<DestinationDto, Guid, DestinationViewModel>(
            entityName: L["Destination"],
            entityNamePlural: L["Destinations"],
            entityResource: TravaloudResource.Destinations,
            ...
            createFunc: async destination =>
            {
                if (!string.IsNullOrEmpty(destination.ImageInBytes))
                {
                    destination.Image = new FileUploadRequest()
                    {
                        ...
                        Name = $"{destination.Name}_{Guid.NewGuid():N}"
                    };
                }
